...
global.comparison = function(variety.to.compare, list.of.genotype.dfs1){
    ...
    main.analysis = lapply(list.of.genotype.dfs1, function(chromo){
        ...
        comparison.list = 1:ncol(chromo) #list of coords of other varieties to compare initial variety to
        comparison.list = comparison.list[-c(1:3, var.coord)]
...
write.csv(comp1, p("rotation1scripts_v4/processed_data/introgressions/int.", file1), row.names = F)
