...
nrow(gt[gt$P.value < 1e-10/totmar(axiomdata),])
...
png(filename=paste("rfvslogplot_quad",quadrant,".png"))
...
png(filename=paste("miscrf_quad",quadrant,".png",sep=""))
...
write.cross(axiomdata4, format="csv", filestem=paste("chromosomes_preordering_quad",quadrant,"/chr_",i,"_pre-ord",sep=""), chr=i)
chrcsv=read.csv(paste("chromosomes_preordering_quad",quadrant,"/chr_",i,"_pre-ord.csv",sep=""),header=T,stringsAsFactors = F)
chrcsv[chrcsv=="AA"]="A"
chrcsv[chrcsv=="BB"]="B"
chrcsv[chrcsv=="AB"]="H"
chrcsv[1,1]=""
chrcsv[2,1]=""
write.csv(chrcsv, paste("chromosomes_preordering_quad",quadrant,"/chr_",i,"_pre-ord.csv", sep=""), row.names=F)
}
...
#setup for amalgamation of ordered chromosomes back together
allchrpost=matrix(nrow=nrow(chrcsv),ncol=1)
#grab row names from preordered chromosome
chrcsv1=read.csv(paste("chromosomes_preordering_quad",quadrant,"/chr_10_pre-ord.csv",sep=""),header=T,stringsAsFactors = F)
bop=cbind(allchrpost,chrcsv1[,1:2])
bop[is.na(bop)]=""
allchrpost=bop

#begin amalgamation
for(i in 2:num_chromosomes){
    chrcsvpost=read.csv(paste("chromosomes_postordering_quad",quadrant,"/chr_",i,"_post-ord.csv",sep=""),header=T,stringsAsFactors = F)
    allchrpost=cbind(allchrpost,chrcsvpost[,-c(1,2)])
}

write.csv(allchrpost,paste("allchrpost_quad",quadrant,".csv",sep=""),row.names=F)

#check if order of individuals is retained in chromosomes after ordering (it is)
chrcsvpost11=read.csv("chromosomes_postordering/chr_11_post-ord.csv",header=T,stringsAsFactors = F)
chrcsvpost12=read.csv("chromosomes_postordering/chr_12_post-ord.csv",header=T,stringsAsFactors = F)
all(chrcsvpost11[,2]==chrcsvpost12[,2])

#read ordered chromosomes back into an rQTL cross object
axiomdataord = read.cross("csv", "./", paste("allchrpost_quad",quadrant,".csv", sep=""), estimate.map=F) #NOTE THE THIRD ARGUMENT — CHANGE BETWEEN flipped_data2.csv AND flipped_data2_pruned.csv ACCORDINGLY
summary(axiomdataord)
plotRF(axiomdataord, alternate.chrid=T)
