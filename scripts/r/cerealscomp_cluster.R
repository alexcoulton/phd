...
if(length(args)!=1){ 
    print("An argument is missing. --args itparam.param")
} else {
    seqparameters=read.csv(args[1],header=F,stringsAsFactors=F)
    zseqstart=seqparameters[1,2]
    print(zseqstart)
    zseqend=seqparameters[2,2]
    print(zseqend)
    lseqstart=seqparameters[3,2]
    print(lseqstart)
    lseqend=seqparameters[4,2]
    print(lseqend)
    ...
    print("loading cerealsdb data")
    cereals=read.csv("/home/ac14037/wheat/rotation1scripts/axiom820_2.csv", header=T)
    cereals$affycode=gsub('-','.',cereals$affycode) #fix affycode field so matches with flipped_data

    zseq=seq(zseqstart,zseqend,0.01)
    print(zseq)
    lodseq=seq(lseqstart, lseqend, 1)
    print(lodseq)
    ...
    for(L in lodseq){
        for(z in zseq){     
            ...
            print("setting up dataframe for population with chromosome data")
            ...
            #####
            #POPULATE MYMAT
            print("beginning mymat population")
            for(p in 1:(length(table(lg[,2]))-10)){
                ...
                ?dir.create
                if(dir.exists(paste("/home/ac14037/wheat/rotation1scripts/chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, sep=""))==F){
                    dir.create(paste("/home/ac14037/wheat/rotation1scripts/chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, sep=""))
                }
                png(filename=paste("/home/ac14037/wheat/rotation1scripts/chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, "/","chromo_", p, ".png", sep=""))
            ...
            print("attempting to output plotRF png file")
            png(filename=paste("/home/ac14037/wheat/rotation1scripts/chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, "/","rf_", rfthres,"_lod_",lodthres, ".png", sep=""))
            plotRF(axiomdata2, alternate.chrid=TRUE)
            dev.off()

            print("writing lgroups data csv file")
            write.csv(mymat,paste("/home/ac14037/wheat/rotation1scripts/linkage_grouping_it/lgroups_rf_",rfthres,"_lod_",lodthres,".csv",sep=""), row.names=F)
            print(paste("end of iteration",z, sep=" "))
        }
    }
}