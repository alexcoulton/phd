...
for(p in 1:(length(table(lg[,2]))-10)){
    ...
    ?dir.create
    if(dir.exists(paste("chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, sep=""))==F){
        dir.create(paste("chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, sep=""))
    }
    png(filename=paste("chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, "/","chromo_", p, ".png", sep=""))
...
png(filename=paste("chromosome_count_plots/rf_", rfthres, "_lod_", lodthres, "/","rf_", rfthres,"_lod_",lodthres, ".png", sep=""))
plotRF(axiomdata2, alternate.chrid=TRUE)
dev.off()

write.csv(mymat,paste("linkage_grouping_it/lgroups_rf_",rfthres,"_lod_",lodthres,".csv",sep=""), row.names=F)
