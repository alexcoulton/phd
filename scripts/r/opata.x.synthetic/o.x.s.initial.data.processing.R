...
o.x.s.array.a = convert.aas.to.rqtl("rotation1scripts_v4/original_data/genotype.data/opata.x.synthetic/opata.x.synthetic.aas.output.array.a.polyhighres.txt")

o.x.s.array.b = convert.aas.to.rqtl("rotation1scripts_v4/original_data/genotype.data/opata.x.synthetic/opata.x.synthetic.aas.output.array.b.polyhighres.txt")

#grab matches again for array a
matches1 = regmatches(o.x.s.array.a[, 2], regexec("OxS.*\\d*\\.", as.character(o.x.s.array.a[, 2])))

matches1.1 = regmatches(as.character(matches1), regexpr("\\d+", as.character(matches1)))

#grab matches again for array b
matches2 = regmatches(o.x.s.array.b[, 2], regexec("OxS.*\\d*\\.", as.character(o.x.s.array.b[, 2])))

matches2.1 = regmatches(as.character(matches2), regexpr("\\d+", as.character(matches2)))


o.x.s.array.a$a.x.c.index = ""
o.x.s.array.a$a.x.c.index = matches1.1

o.x.s.array.b$a.x.c.index = ""
o.x.s.array.b$a.x.c.index = matches2.1

#remove samples from array a that are not on array b
o.x.s.array.a = o.x.s.array.a[-which(!o.x.s.array.a$a.x.c.index %in% o.x.s.array.b$a.x.c.index), ]

#TODO: merge array dataframes. find parent genotyping information for markers on array b

merged.array.df = cbind(o.x.s.array.a, o.x.s.array.b[3:ncol(o.x.s.array.b)])
...
rs = dbSendQuery(mydb, "SELECT * FROM 35k_array_oct2016_named WHERE Var_col LIKE '%Opata%'") #grab Avalon genotyping data for 35k array
rs2 = fetch(rs, n=-1)

#grab cadenza info
ca = dbSendQuery(mydb, "SELECT * FROM 35k_array_oct2016_named WHERE Var_col LIKE '%Synthetic%'") #grab Avalon genotyping data for 35k array
ca2 = fetch(ca, n=-1)
ca2 = ca2[ca2$Var_col == "Synthetic", ]
...
g = rs2$Matrix_value[match(colnames(merged.array.df)[3:ncol(merged.array.df)], rs2$Probe_row)] #grab genotyping data for opata

merged.array.df[1, 3:ncol(merged.array.df)] = g
merged.array.df[1, 2] = "Opata"

#prepare cadenza genotyping data
g = ca2$Matrix_value[match(colnames(merged.array.df)[3:ncol(merged.array.df)], ca2$Probe_row)] #grab genotyping data for synthetic

merged.array.df[2, 3:ncol(merged.array.df)] = g
merged.array.df[2, 2] = "Synthetic"
...
write.csv(merged.array.df3, "rotation1scripts_v4/processed_data/genotypes/opata.x.synthetic/o.x.s.flipped.csv", row.names = F)
